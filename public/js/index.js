...
//console.log('render graph');
			...
			for (var i=0; i < data.length; i++) {
				...
				label.push(i);
				for (var k in data[i]) {
					if (typeof labelData[k] === "undefined") {
						labelData[k] = [];
					}
					labelData[k].push(data[i][k]);
				}
...
//				labelData3.push(parseInt(Math.random()*200));
			...
			labelData1 = labelData["science"];
			labelData2 = labelData["chronology"];
			labelData3 = labelData["language"];
...
//				data:  {"cid": 2},
				...
				success: function(response) {
					...
					for(cat in response.categories) {
						for (var y=0; y < response.categories[cat].length; y++) {

							index.populateCell(i,response.categories[cat][y]['tweet'],cat);
						}
						i++;
					}
					
					index.renderGraph(response.graphData);
				},
		...
		populateCell: function(index, json, title) {
